...
def bit_frequency_test(e: str) -> float:
    ...
    Arguments:
                e: sequence string
    Returns:
                float: P value of the sequences
    """
    try:
        n = len(e)
        s = 0
    ...
    try:
        ...
        for i in range(0, n - 1):
            if e[i] != e[i + 1]:
                vn += 1
        pv = math.erfc(
            abs(vn - 2 * n * s * (1 - s)) / (2 * math.sqrt(2 * n) * s * (1 - s))
        )
        return pv
    except Exception as e:
        print(f"Произошла ошибка во время теста: {e}")
...
def find_max_consecutive_ones(block: str) -> int:
    """
    The function finds the maximum number of consecutive units
    
    Arguments:
                block:  sequence block
    Returns:
                int: the maximum number of consecutive units
    """
    max_ones = 0
    current_ones = 0
    for bit in block:
...
def unit_sequence_test(e: str) -> float:
    """
    The function searches for the longest sequence of units in a block
